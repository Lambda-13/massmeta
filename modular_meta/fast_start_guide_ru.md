...
Преполагается, что читатель понимает основы GitHub и не боится здешнего языка. Вас предупредили.
...
Обязательно помечайте их **все** до единого этим уникальным индентификатором, в том числе новые файлы мне модуля (например TGUI).
...
| Папка/Файл                         | Описание
| ---------------------------------- | ------------------------------- |
| **features**                    📁 | Наши новые фичи, что добавили   |
| **reverts**                     📁 | Убранные фичи из основного кода |
| **tools**                       📁 | Инструменты для модульного кода |
| **modular_include.dm**          📄 | Общий include модулей           |
| **modularization_guide_ru.md**  📝 | Полный гайд модульности         |
...
При использованни в 1 вашем файле:
...
❌ Очень трудно следить за оригинальными объектами и функциями. (частично нивелированно авто-тестами)
...
- **Пример модульного добавления фичи в функцию**
  ...
  Дополняете оригинальный объект новой переменной через метод переопределения (мне наследники также теперь обладают этой переменной):
...
Н иногда придется записывать так: `{/* MASSMETA EDIT START */}`.
...
0. Обновить master-ветку.
1. Создай новую ветку (от мастера) под твою фичу.
