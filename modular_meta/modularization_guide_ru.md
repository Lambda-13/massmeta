...
**Основная цель – это как можно проще положить множество наши дополнений в огромный и постоянно меняющийся основной код, чтобы потом было как можно проще подхватывать нововведения Апстрима.**

Напоминает моды из различного рода игр? – по сути оно и есть.

Во многих репозиториях Space Station 13 есть уже "своя модульность". От репозитория к репозиторию она разная, но здесь вы скорее всего увидите нечто похожее. Возможно даже она будет лучше.
...
- **Добавление**
  ...
  Просто вставляетете конструкцию подобно нижней.
...
- **Удаление**
  ...
  **Комментируете**, тем изменяя их, обязательно **все удаляемые строчки кода**. Мы добиваемся того, что при следующей обнове мы увидем все вмешательства апстрима в свой оригинальный код, что мы убрали.
...
В нашем проекте как раз есть папка `modular_meta/` под наши "модицикации", каждая лежит в своей папке с уникальным названием.
...
В `readme.md` вашего модуля можете дописать важные пояснения и нюансы. Например в нем вы можете указать ссылку на PR или серию PR-ов, который откатываевается/портируется.
...
Сперва все файлы из папки `code/` добавляются в `includes.dm` модуля, затем уже он сам в `modular_include.dm`, а тот уже в `tgstation.dme`.
...
В вашем модуле могут находиться сразу несколько мелких фич на общую тематику, для этого может потребоваться на одна папка.

#### Директивы препроцессора ▶️
...
За счёт отсуствия изменений в основном коде, не нарушается череда оригинальных коммитов, и тем самым мы **не допускаем Конфликты Слияния**.
...
- **Пример модульного переопределения объекта**
  ...
  Для этого создайте новый файл в папке `code/` вашего модуля: `code/m_paintings.dm`.
